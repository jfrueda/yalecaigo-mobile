...
class _ProviderReputationPageState extends State<ProviderReputationPage> {
  ...
  Future<void> _refresh() async {
    final nextFuture = _service.providerReputation(widget.providerId);
    if (!mounted) return;
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        _metric('Actividades finalizadas', data['completed_services']),
                        _metric('Cancelaciones atribuibles', data['cancelled_services']),
                        _metric('No presentado', data['no_show_count']),
                        _metric('Cumplimiento', '${data['completion_rate'] ?? '0.00'} %'),
                        _metric('Tasa de cancelación', '${data['cancellation_rate'] ?? '0.00'} %'),
                      ],
                    ),
                  ),
                ),
                  ...
                  ...reviews.map(
                    (review) => Card(
                      child: ListTile(
                        ...
                        title: Text(
                          review['author_display_name']?.toString() ?? 'Usuario',
                        ),
                        subtitle: Text(
